...
public class FindGraphPath {
	...
	final static String loop = "Not getting to a termination node";
	static boolean negativeWeights = false;
	static boolean forbiddenInput = false;
	
	public static void main(String[] args){
		
		String fileName = "words.txt"; // change this to the desired file name
		File file = new File(fileName);
		fileAddress = file.getAbsolutePath();
		
		createGraph(); //write the text file into an arraylist of nodes
			...
			while (scan.hasNext()) {
				...
				//check if forbidden strings are inserted
					...
					if (nodeIndex==-1){
						...
						Arrow newArrow = new Arrow(Double.parseDouble(edge[1]),edge[2]);
						Node nodeWithNewArrow = nodesList.get(nodeIndex).addArrow(newArrow);
						nodesList.set(nodeIndex,nodeWithNewArrow);
					}
				}
			}
	...
	static void findTerminationNodes(Node node, Double chance){
	...
	private static void addTerminationNode(String name, Double chance) {
		chance=chance*100;
		
		//check if the node already exists in the terminationNodes list.
		int TerNodeIndex = searchListTerNode(name,terminationNodes);
		
		if(TerNodeIndex==-1){
			//if it doesn't appear in the termination node list, we add it
			terminationNodes.add(new TerNode(name,chance));
		}
		else{
			...
			Double newChance = chance+terminationNodes.get(TerNodeIndex).chance;
			terminationNodes.set(TerNodeIndex, new TerNode(name,newChance));
		}
	...
	static int searchListNodes(String name, ArrayList<Node> list){
